...
void SysTick_Handler(void)
{
	if(tx_count > 10) {
		...
		if(index > 7) {
			index = 0;
			tx_config.sync[0] = 0x91;
		}
		tx_config.sync[0] = tx_config.sync[0] + index;
		pipe = (prf_pipe_t)(0x1 << index);
		panchip_prf_set_addr(tx_config.sync, tx_config.sync_length, pipe, PRI_RF_MODE_SEL_TRX);
		tx_count = 0;
	} else {
		tx_count++;
	}
...
int main(void)
{
	...
	SYS_TEST("prf tx multi pipe sample init\n");
