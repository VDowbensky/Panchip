...
void event_tx_fun(void)
{
	...
	SYS_TEST("len:%d,pipe %d,rx data:",rx_payload.data_length, panchip_prf_get_pipe());
...
void panchip_prf_multi_addr_set(void)
{
	prf_pipe_t pipe;
	uint8_t index;

	for(index = 1; index <= 7; index++) {
		rx_config.sync[0] = rx_config.sync[0] + index;
		pipe = (prf_pipe_t)(0x1 << index);
		panchip_prf_set_addr(rx_config.sync, rx_config.sync_length, pipe, PRI_RF_MODE_SEL_TRX);
	}
}
...
int main(void)
{
	...
	panchip_prf_multi_addr_set();

	if(rx_config.trf_type == PRF_TRF_B250K)
	{
