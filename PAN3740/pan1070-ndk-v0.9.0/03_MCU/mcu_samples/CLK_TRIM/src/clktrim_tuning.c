...
static void TRIM_PrintInfoCase2(void)
{
    SYS_TEST("\n");
    SYS_TEST("+--------------------------------------------------------------------------------------+ \n");
...
void TRIM_TuningProcess(uint32_t func,uint8_t flag,uint8_t code,uint32_t cnt)
{
	uint32_t hclk;
	char s[32];
	...
	/*set error range*/
	TRIM_SetErrRange(TRIM,32);
	/*start tuning*/
	TRIM_StartTuning(TRIM,fun_en);

	/*wait stop & clear flag*/
	while(!TRIM_IsIntStatusOccured(TRIM,check_flag)){}
	TRIM_ClearIntStatusMsk(TRIM,check_flag);

	uint32_t real_cnt = TRIM_GetRealCnt(TRIM);
	float cur_freq = FREQ_32MHZ;
	cur_freq /= real_cnt;
	cur_freq *=	calc_cnt;
		SYS_TEST("real cnt :%d,c code:%x,f code:%x,cur_freq:%f\n",TRIM_GetRealCnt(TRIM),TRIM_GetCoarseCode(TRIM),
		TRIM_GetFineCode(TRIM),cur_freq);
}

void TRIM_CPTuningProcess(uint32_t func,uint8_t flag,uint32_t bit_width,uint32_t code_value,uint32_t calc_cnt)
{
	uint32_t hclk;
	uint32_t fun_en,check_flag;
	/*set bit width value*/
...
T_TRIM_TEST_RESULT TRIM_TuningCase2(void)
{
	char c;
	...
	while(true){
		...
		switch(c){
			...
			case 'A':
				...
				TRIM_TuningProcess(FUNC_SELECT_COARSE_TUNING,EARLY_TERMINATION_OPEN,3,20);
				break;
			case 'b':
			case 'B':
				SYS_TEST("fine tuning test select\n");
				TRIM_TuningProcess(FUNC_SELECT_FINE_TUNING,EARLY_TERMINATION_OPEN,7,50);
				break;
			case 'c':
			case 'C':
				SYS_TEST("all tuning test select\n");
				TRIM_CPTuningProcess(FUNC_SELECT_COARSE_TUNING,EARLY_TERMINATION_CLOSE,3,0X8,2);
				TRIM_CPTuningProcess(FUNC_SELECT_FINE_TUNING,EARLY_TERMINATION_CLOSE,7,0X80,2);
				break;
			case 0x1b:
				goto OUT;
				break;
