...
pan_prf_config_t template_config = {
	...
	.rx_timeout			= PRF_RX_TIMEOUT,		//us
	.rf_channel			= PRF_CHANNEL,
	.tx_no_ack			= PRF_TX_NO_REVCV_ACK,
	.trf_type			= (prf_trf_t)PRF_TRANS_TYPE,
	.rx_length			= PRF_RX_PAYLOAD_LENGTH,
	.sync_length		= PRF_SYNC_WORD_LENGTH,
	.crc_include_sync	= PRF_CRC_INCLUDE_SYNC,
	.src_include_sync	= PRF_WHITE_INCLUDE_SYNC,
	.sync				= { 0x91, 0xd3, 0x91, 0xd3 },
	.pid_manual_flag	= PRF_PID_MANNUAL_EN,
	.tx_power			= PRF_TX_POWER,
	.pipe				= (prf_pipe_t)PRF_PIPE_NUM,
};
...
{
#if(PRF_TRX_SELECT == PRF_TX_MODE)
	#if(PRF_WORK_MODE == PRF_MODE_ENHANCE)
	if (usr_pid >= 3){
		usr_pid = 0;
	}
...
void event_rx_fun(void)
{
	...
	data_printf(rx_payload.data, rx_payload.data_length);
#else
	if (template_config.work_mode == PRF_MODE_ENHANCE)
	{
	...
	else
	{
		...
		rx_payload.data_length = panchip_prf_data_rec(&rx_payload);
		SYS_TEST("len:%d,rx data:",rx_payload.data_length);
		data_printf(rx_payload.data, rx_payload.data_length);
		panchip_prf_trx_start();
	}
#endif
...
{
	if (rx_config.trx_mode == PRF_RX_MODE)
	{
		panchip_prf_trx_start();
	}
}
...
int main(void)
{
	...
	panchip_prf_init(&template_config);
	if(template_config.trf_type == PRF_TRF_B250K)
	{
		panchip_prf_b250k_set_channel(3);
	}
	else
	{
		panchip_prf_set_chn(template_config.rf_channel);
	}
	...
	SYS_TEST("prf template sample init\n");
