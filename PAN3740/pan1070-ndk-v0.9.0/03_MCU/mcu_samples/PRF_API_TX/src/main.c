...
static volatile uint8_t pid_num;
...
void init_payload_data(void)
{
	...
	memcpy(tx_payload.data, send_data, PRF_DATA_MAX_SIZE);
}

__ramfunc uint16_t tx_payload_length_select(void)
{
	char length;
	uint16_t data_length = 0;

	SYS_TEST("choose data length:\n");
	SYS_TEST("0-->no ack payload\n");
	SYS_TEST("1-->1byte payload\n");
	SYS_TEST("2-->32byte payload\n");
	SYS_TEST("3-->63byte payload\n");
	SYS_TEST("4-->64byte payload\n");
	SYS_TEST("5-->255byte payload\n");
	length = getchar();
...
__ramfunc void event_tx_fun(void)
{
	if (!rate_test) {
		...
		if ((PRI_RF_IsTxNoAckEn(PRI_RF))
		    && (usr_config.work_mode == PRF_MODE_ENHANCE)
		    && (limit_rate_test)) {
			panchip_prf_trx_start_fast();
		} else if ((usr_config.work_mode != PRF_MODE_ENHANCE) && (limit_rate_test)) {
			panchip_prf_trx_start_fast();
		}
	}
}
...
__ramfunc void event_rx_fun(void)
{
	...
	if (rate_test) {
		rf_isr_cnt.rx_cnt++;
		if ((usr_config.work_mode == PRF_MODE_ENHANCE) && (!PRI_RF_IsTxNoAckEn(PRI_RF))) {
			if (limit_rate_test) {
				panchip_prf_trx_start_fast();
			}
		}
...
void prf_api_test_print(void)
{
	...
	SYS_TEST("G: TEST RF API panchip_prf_set_data\n");
	SYS_TEST("H: TEST RF API panchip_prf_mode_conf_set\n");
	SYS_TEST("I: TEST RF API panchip_prf_carrier_start/stop\n");
...
void prf_api_test_panchip_prf_set_chn(void)
{
	...
	while (channel <= 2480) {
		...
		while (send_cnt--) {
			panchip_prf_pid_cfg(&usr_config, pid_num++);
			if (pid_num >= 3) {
				pid_num = 0;
			}
			panchip_prf_trx_start();
...
void prf_api_test_panchip_prf_set_tx_pwr(void)
{
	...
	while (tx_pwr <= 7) {
		send_cnt = 3;
		panchip_prf_set_chn(channel);
		SYS_TEST("ch %d set tx power %d dbm\n", channel, tx_pwr);
		panchip_prf_set_tx_pwr(tx_pwr);
...
void prf_api_test_panchip_prf_set_addr(void)
{
	...
	panchip_prf_set_addr(addr, 5, PRF_PIPE0, PRI_RF_MODE_SEL_TX);
	panchip_prf_trx_start();
	while (!tx_done) {
	}
	tx_done = false;
	...
	panchip_prf_set_addr(addr_raw, 4, PRF_PIPE0, PRI_RF_MODE_SEL_TX);
}

void prf_api_test_panchip_prf_set_data(void)
{
	panchip_prf_payload_t test_tx_payload = {
		.data_length = 5,
	...
	memcpy(test_tx_payload.data, send_data, PRF_DATA_MAX_SIZE);

	panchip_prf_set_chn(channel);

	SYS_TEST("send 1 data\n");
	test_tx_payload.data_length = 1;
	panchip_prf_set_data(&test_tx_payload);
	panchip_prf_trx_start();
	while (!tx_done) {
	}
	tx_done = false;
	...
	TIMER_Delay(TIMER0, 50000);
	SYS_TEST("send 64 data\n");
	test_tx_payload.data_length = 64;
	panchip_prf_set_data(&test_tx_payload);
	panchip_prf_trx_start();
	while (!tx_done) {
	}
	...
	TIMER_Delay(TIMER0, 50000);
	SYS_TEST("send max 255 data\n");
	test_tx_payload.data_length = 255;
	panchip_prf_set_data(&test_tx_payload);
	panchip_prf_trx_start();
	while (!tx_done) {
	}
...
void prf_api_test_panchip_prf_carrier_start_stop(void)
{
	uint16_t channel = 2402;

	while (1) {
		SYS_TEST("carrier start, channel %d\n", channel);
		panchip_prf_carrier_start(channel);
		TIMER_Delay(TIMER0, 20);
		channel += 1;
		if (channel > 2480) {
...
void prf_api_test_panchip_prf_set_ack_data(void)
{
	...
	SYS_TEST("r02:%x\n",PRI_RF->R02_TMR_CTL);
	tx_payload.data_length = 1;
	prf_debug_pin_enable();
	while (send_cnt < 10) {
		SYS_TEST("set tx payload length = %d\n", tx_payload.data_length);
		...
		rx_done = false;
		if (send_cnt < 8) {
			tx_payload.data_length += 5;
		} else {
			tx_payload.data_length = 63;
		}
		send_cnt += 1;
	}
...
void prf_api_test_event_pid_err_fun(void)
{
	...
	prf_debug_pin_enable();
	while (send_cnt < 10) {
		panchip_prf_pid_cfg(&usr_config, 0);
		panchip_prf_trx_start();
		while (!rx_done) {
		}
		rx_done = false;
...
void prf_api_test_panchip_prf_trx_trans_time(void)
{
	uint16_t channel = 2402;
	uint16_t send_cnt = 0;
	uint16_t trans_wait_time = 80;
...
__ramfunc void packet_acceptance_rate_test_normal(void)
{
	uint16_t send_cnt = 0, test_cnt = 0;
	panchip_prf_payload_t test_tx_payload = {
		.data_length = 5,
		.data = { 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07 },
	};
	...
	prf_debug_pin_enable();
	data_length = tx_payload_length_select();
	SYS_TEST("data length = %d\n", data_length);

	test_tx_payload.data_length = data_length;
	panchip_prf_set_data(&test_tx_payload);
...
__ramfunc void packet_acceptance_rate_test_enhance(void)
{
	uint16_t channel = 2402;
	uint16_t send_cnt = 0, test_cnt = 0;
	...
	tx_payload.data_length = tx_payload_length_select();
	if (tx_payload.data_length > 64) {
...
__ramfunc void limit_packet_acceptance_rate_test_enhance(void)
{
	uint16_t channel = 2402;
	...
	if (tx_payload.data_length > 64) {
		...
		if (tx_payload.data_length == 0) {
			panchip_prf_set_tx_noack(true);
		} else {
			PRI_RF_IntMask(PRI_RF, R01_INT_TX_IRQ_MASK_Msk, ENABLE);
			SYS_TEST("set tx payload length = %d\n", tx_payload.data_length);
		}
	}
	panchip_prf_set_data(&tx_payload);
...
int main(void)
{
	...
	while (1) {
		...
		case 'F':
			...
			break;
		case 'G':
			prf_api_test_panchip_prf_set_data();
			break;
		case 'H':
			prf_api_test_panchip_prf_mode_conf_set();
			break;
		case 'I':
			prf_api_test_panchip_prf_carrier_start_stop();
			break;
		case 'J':
