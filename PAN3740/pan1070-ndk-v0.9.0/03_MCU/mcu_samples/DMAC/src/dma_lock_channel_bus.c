/********************************************************* * 
@file		:dma_lock_channel_bus.c 
@brief  	:3dma channel transfer form memory to peripheral spi with normal dma channel config,both dma channel 2 and bus are locked
@author 	:zhongfeng
@version	:v1.0 
@date 		:19/08/29
...
uint32_t DMA_ChannelBusLockedTranferCase22(void)
{
    ...
    TxConfigTmp2.LockChannel     = ENABLE;
    TxConfigTmp2.LockChannelLevel= DMAC_LockLevel_Tfr_Complete;
    TxConfigTmp2.LockBus         = ENABLE;
    TxConfigTmp2.LockBusLevel    = DMAC_LockLevel_Tfr_Complete;
    ...
    while(TestCnt--){
        ...
        DMAC_StartChannel(DMA, gTxChNum,gDmaSendData,(void*)&(SPI0->DR),DATA_LENTH_TRANSFER_0/4);
