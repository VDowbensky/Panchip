...
uint32_t DMA_HardwareMultiBlockTestCase29(void)
{
...
#else
	char c;
	uint8_t srcAutoReload,dstAutoReload;
    ...
    SYS_TEST("\n");
    SYS_TEST("|    Press key to choose auto reload config :                                 | \n");
    SYS_TEST("|                                                                             | \n");
    SYS_TEST("|    Input '0'    src auto reload,dst configuration                           | \n");
    SYS_TEST("|    Input '1'    dst auto reload,src configuration                           | \n");
    SYS_TEST("|    Input '2'    src auto reload,dst auto reload                             | \n");
    SYS_TEST("|                                                                             | \n");
	
	switch(c = getchar())
	{
		case '0':srcAutoReload = 1,dstAutoReload = 0;break;
		case '1':srcAutoReload = 0,dstAutoReload = 1;break;
		case '2':srcAutoReload = 1,dstAutoReload = 1;break;
		default:srcAutoReload = 0,dstAutoReload = 0;break;
	}
	...
	TxConfigTmp.AutoReloadSrc   = srcAutoReload;
	TxConfigTmp.AutoReloadDst   = dstAutoReload;
...
//    while(TestCnt--)
    {
		...
		DMAC_ClrIntFlagMsk(DMA,gTxChNum,DMAC_FLAG_INDEX_BLK);
