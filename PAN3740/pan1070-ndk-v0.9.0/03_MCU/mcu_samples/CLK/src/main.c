 ...
 * @version  V1.00
 * $Date:    19/11/12 16:42 $
 * @brief    WDT Sample Code. To test WDT Timeout Interval Period Selection,
 *           Clock Source Selection, Interrupt Mode, Reset Mode, and Wakeup
 *           Signal, etc.
 * @note
...
void CLK_PrintSampleInfo()
{
    SYS_TEST("\n");
    SYS_TEST("+--------------------------------------------------------------------------------------+ \n");
    SYS_TEST("|                      PAN107X CLK Sample Code.                                        | \n");
    SYS_TEST("+--------------------------------------------------------------------------------------+ \n");
    SYS_TEST("|    Press key to start specific testcase:                                             | \n");
    SYS_TEST("|                                                                                      | \n");
    SYS_TEST("|    Input '0'    Testcase 0: Clk source select RC32M                                  | \n");
    SYS_TEST("|    Input '1'    Testcase 1: Clk source select XTH32M.                                | \n");
    SYS_TEST("|    Input '2'    Testcase 2: Clk source select DPLL.                                  | \n");
    SYS_TEST("|    Input '3'    Testcase 3: 32K Clk Source select RC.                                | \n");
    SYS_TEST("|    Input '4'    Testcase 4: 32K Clk Source select XTL.                               | \n");
    SYS_TEST("|    Input '5'    Testcase 5: Clk divider test.                                        | \n");
    SYS_TEST("|    Input '6'    Testcase 6: Clk register default value check.                        | \n");
    SYS_TEST("+--------------------------------------------------------------------------------------+ \n");
}
...
int main(void)
{
    ...
    while (c)
    {
        ...
        case '0':
            CLK_TestFunctionEnter(0);
            break;
        case '1':
            CLK_TestFunctionEnter(1);
            break;
        case '2':
            CLK_TestFunctionEnter(2);
            break;
        case '3':
            CLK_TestFunctionEnter(3);
            break;
        case '4':
            CLK_TestFunctionEnter(4);
            break;
        case '5':
            CLK_TestFunctionEnter(5);
            break;
        case '6':
            CLK_TestFunctionEnter(6);
            break;
        default:
