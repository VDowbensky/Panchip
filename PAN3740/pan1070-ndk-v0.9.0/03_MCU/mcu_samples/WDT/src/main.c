...
void WDT_PrintSampleInfo()
{
    SYS_TEST("\n");
    SYS_TEST("+--------------------------------------------------------------------------------------+ \n");
    SYS_TEST("|                      PAN107X WDT Sample Code.                                        | \n");
    SYS_TEST("+--------------------------------------------------------------------------------------+ \n");
    SYS_TEST("|    Press key to start specific testcase:                                             | \n");
    SYS_TEST("|                                                                                      | \n");
    SYS_TEST("|    Input '0'    Testcase 0: Register Default Value Check.                            | \n");
    SYS_TEST("|    Input '1'    Testcase 1: Timeout Interval Period Selection.                       | \n");
    SYS_TEST("|    Input '2'    Testcase 2: Clock Source Selection.                                  | \n");
    SYS_TEST("|    Input '3'    Testcase 3: Interrupt Mode.                                          | \n");
    SYS_TEST("|    Input '4'    Testcase 4: Reset Mode.                                              | \n");
    SYS_TEST("|    Input '5'    Testcase 5: Wakeup Signal.                                           | \n");
    SYS_TEST("+--------------------------------------------------------------------------------------+ \n");
}
...
int main(void)
{
    ...
    SYS_TEST("\nCPU @ %dHz\n", SystemCoreClock);
	if (CLK->XTL_CTRL_3V & CLK_XTLCTL_XTL_CAP_EN_Msk_3v)
		SYS_TEST("XTL_CAP_EN value is retentioned \n");
	else
		SYS_TEST("XTL_CAP_EN value is reseted \n");
    WDT_TestModuleInit();
	WDT_ResetRegionSelect();
    WDT_PrintSampleInfo();
    ...
    while( c )
    {
        ...
        switch(c)
        {
            ...
            case '0':
                WDT_TestFunctionEnter(0);
                break;
            /* 1. Timeout Interval Period Selection */
            case '1':
                WDT_TestFunctionEnter(1);
                break;
            /* 2. Clock Source Selection */
            case '2':
                WDT_TestFunctionEnter(2);
                break;
            /* 3. Interrupt Mode */
            case '3':
                WDT_TestFunctionEnter(3);
                break;
            /* 4. Reset Mode */
            case '4':
                WDT_TestFunctionEnter(4);
                break;
            /* 5. Wakeup Signal */
            case '5':
                WDT_TestFunctionEnter(5);
                break;
            default:
