...
#include "sub_1g_param.h"

/*
 * flag that indicate if a new packet is received.
*/
static volatile int packet_received = RADIO_FLAG_IDLE;

/*
 * flag that indicate if transmision is finished.
*/
static volatile int packet_transmit = RADIO_FLAG_IDLE;
...
{
#ifdef _USE_RF_RST_IO_
	PORT_ResetBits(GPIO_PORT_RST, GPIO_PIN_RST);
	rf_delay_us(10);
	PORT_SetBits(GPIO_PORT_RST, GPIO_PIN_RST);
#else
...
RF_Err_t rf_set_tx_power(uint8_t tx_power)
{
	...
	tx_power = (tx_power > RF_MAX_RAMP ? RF_MAX_RAMP : tx_power);
	tx_power = (tx_power < RF_MIN_RAMP ? RF_MIN_RAMP : tx_power);
 ...
 * @return result
 */
RF_Err_t rf_set_preamble_len(uint16_t len)
{
    uint8_t temp[2] = {len & 0xff, (len >> 8) & 0xff};
...
/**
 * @brief set preamble
 * @param[in] preamble time
 * @return result
 */
RF_Err_t rf_set_preamble_time(uint16_t ms)
{
	uint32_t chirp_time = 0;
	uint8_t sf,bw;
	uint16_t reg,len;	
	
	if(ms == 0)
	{
		rf_set_preamble_len(DEFAULT_PREAMBLE_LEN);
		return OK;
	}

	bw = rf_get_bw();
	sf = rf_get_sf();
	chirp_time = rf_get_chirp_time(bw,sf);
	
	reg = ms*1000 / chirp_time + 1;
	
	len = DEFAULT_PREAMBLE_LEN > reg ? DEFAULT_PREAMBLE_LEN : reg;

	RF_ASSERT(rf_set_preamble_len(len));
	return OK;
}
...
RF_Err_t rf_set_auto_sf_rx_off(void)
{
    ...
    RF_ASSERT(rf_set_preamble_len(DEFAULT_PREAMBLE_LEN));
