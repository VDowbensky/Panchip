...
void EFUSE_WriteByte(EFUSE_T* efuse, uint32_t addr, uint8_t data)
{
    ...
    efuse->EF_ADDR   = addr;
	  efuse->EF_DAT    = data | (data<< 8); //efuse data; bit0-7:data, bit8-15:data confirm
    efuse->EF_TRG   |= EFUSE_TRG_EFUSETRG_GO_Msk;
    while(efuse->EF_TRG){}
...
bool EFUSE_Write(EFUSE_T* efuse, uint32_t addr, uint8_t* data, uint16_t len)
{
    ...
    for (size_t i = 0; i < len; i++)
    {
        efuse->EF_CMD    = EFUSE_CMD_PROGRAM;
        efuse->EF_ADDR   = addr + i;
        efuse->EF_DAT    = data[i] | (data[i]<< 8); //efuse data; bit0-7:data, bit8-15:data confirm;
        efuse->EF_TRG   |= EFUSE_TRG_EFUSETRG_GO_Msk;
        while(efuse->EF_TRG){}
...
{
    if ((addr < 0x7C) || (addr > 0x7F) || (addr + len > 0x80))
    {
        return false;
    }
