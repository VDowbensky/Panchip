                  ...
                  struct ble_sm_result *res)
{
    ...
    if (res->app_status == 0) {
        ...
        if (res->app_status == 0) {
         ...
         #if PAN_BLE_HS_FIXED_SM_SEC_REQ_BUG_EN
            if(res->app_status == BLE_HS_EALREADY){
                res->app_status = 0;
            }
         #endif
        }
...
#if PAN_BLE_HS_SM_API_EN && NIMBLE_BLE_SM
...
{
    ble_hs_cfg.sm_oob_data_flag = oob_data_flag;
}

void ble_sm_set_our_key_dist(uint8_t key_mask)
{
    ble_hs_cfg.sm_our_key_dist = key_mask;
}

void ble_sm_set_peer_key_dist(uint8_t key_mask)
{
    ble_hs_cfg.sm_their_key_dist = key_mask;
}
#endif
