 ...
 */
#ifdef IP_101x
static os_membuf_t *ble_hs_hci_frag_data;
#else
                  ...
                  void *rsp, uint8_t rsp_len)
{
    ...
    BLE_HS_LOG(DEBUG, "hci cmd:%02x,%04x", BLE_HCI_OGF(opcode), BLE_HCI_OCF(opcode));

    if(rc != 0) {
        BLE_HS_LOG(ERROR, "hci err:%02x,%04x, rc:%02x", BLE_HCI_OGF(opcode), BLE_HCI_OCF(opcode), 0xff & rc);
    }
