...
void usbd_event_ep0_setup_complete_handler(uint8_t busid, uint8_t *psetup)
{
    ...
    /* Set long length packet (LLP) flag when host asks for data size larger than EP0 MPS (MaxPacketSize, 64B) */
    if (setup->wLength > USB_CTRL_EP_MPS) {
        g_llp_flag = true;
        USB_LOG_DBG("EP0 Set llp\r\n");
    }
...
void usbd_event_ep0_in_complete_handler(uint8_t busid, uint8_t ep, uint32_t nbytes)
{
    ...
    if ((g_llp_flag == true) && (g_usbd_core[busid].ep0_data_buf_residue <= 64)) {
        /* Clear LLP flag when the last packet is going to be sent */
        g_llp_flag = false;
    }
