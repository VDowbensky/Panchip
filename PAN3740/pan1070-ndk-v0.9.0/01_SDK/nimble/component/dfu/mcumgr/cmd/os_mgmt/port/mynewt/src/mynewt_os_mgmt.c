...
struct ble_npl_callout  os_mgmt_reset_callout;

#if 1
static void os_mgmt_reset_tmo(struct ble_npl_event *ev)
{
	mcu_reboot();
}
...
{
	ble_npl_callout_init(&os_mgmt_reset_callout, nimble_port_get_dflt_eventq(),
                          os_mgmt_reset_tmo, NULL);
	
	ble_npl_callout_reset(&os_mgmt_reset_callout, OS_MS_TO_TICK(delay_ms));
