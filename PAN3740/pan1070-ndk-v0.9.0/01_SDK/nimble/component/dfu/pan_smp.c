...
void mgmt_evq_set(struct ble_npl_eventq *evq)
{
    g_smp_evq = evq;
}
...
static void *smp_alloc_rsp(const void *req, void *arg)
{
   struct os_mbuf *m;
   struct os_mbuf *rsp;
...
static void smp_trim_front(void *m, size_t len, void *arg)
{
    os_mbuf_adj(m, len);
}

static void smp_reset_buf(void *m, void *arg)
{
    if (!m) {
        return;
    }
...
static void smp_free_buf(void *m, void *arg)
{
    if (!m) {
        return;
    }
...
static int smp_init_reader(struct cbor_decoder_reader *reader, void *m,
		void *arg)
{
...
static int smp_init_writer(struct cbor_encoder_writer *writer, void *m,
		void *arg)
{
    struct cbor_mbuf_writer *cmw;
 ...
 */
static struct os_mbuf *smp_rsp_frag_alloc(uint16_t frag_size, void *arg)
{
...
int smp_tx_rsp(struct smp_streamer *ns, void *rsp, void *arg)
{
 ...
 */
static int smp_process_packet(struct smp_transport *st)
{
...
int smp_rx_req(struct smp_transport *st, struct os_mbuf *req)
{
    ...
    rc = os_mqueue_put(&st->st_imq, nimble_port_get_dflt_eventq(), req);
    if (rc) {
        goto err;
    }
...
static void smp_event_data_in(struct ble_npl_event *ev)
{
...
void ble_smp_eventq_init(struct ble_npl_eventq *evq)
{
...
void smp_pkg_init(void)
{
    ...
    mgmt_evq_set(nimble_port_get_dflt_eventq());
}
