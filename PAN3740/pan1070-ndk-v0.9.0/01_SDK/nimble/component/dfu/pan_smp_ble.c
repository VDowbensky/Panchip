    ...
    switch (ctxt->op) {
        case BLE_GATT_ACCESS_OP_WRITE_CHR:
            ...
            if (OS_MBUF_USRHDR_LEN(ctxt->om) >= sizeof (conn_handle)) {
                 ...
                 * it.
                 */
                m_req = os_msys_get_pkthdr(OS_MBUF_PKTLEN(ctxt->om), sizeof (conn_handle));
                if (!m_req) {
                    return BLE_ATT_ERR_INSUFFICIENT_RES;
                }
                rc = os_mbuf_appendfrom(m_req, ctxt->om, 0, OS_MBUF_PKTLEN(ctxt->om));
                if (rc) {
                    return BLE_ATT_ERR_INSUFFICIENT_RES;
                }
            }
...
uint16_t smp_ble_get_mtu(struct os_mbuf *req) {
 ...
 */
static void smp_ble_event_data_in(struct ble_npl_event *ev)
{
    ...
    while ((m_resp = os_mqueue_get(&g_smp_ble_mq)) != NULL) 
    {
        ...
        ble_gattc_notify_custom(conn_handle, g_ble_smp_attr_handle, m_resp);
    }
}

static int smp_ble_out(struct os_mbuf *om)
{
    int rc;

    rc = os_mqueue_put(&g_smp_ble_mq, nimble_port_get_dflt_eventq(), om);
    if (rc != 0) {
        goto err;
    }
 ...
 * @return 0 on success; non-zero on failure
 */
int smp_ble_gatt_svr_init(void)
{
    int rc;
