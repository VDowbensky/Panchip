...
static void spi_slave_receive_send_test(void)
{
    ...
    HAL_SPI_ReceiveData(&SPI0_OBJ, slave_receive_buf, slave_receive_send_cnt, 0);
    HAL_SPI_DeInit(&SPI0_OBJ);

    APP_LOG_INFO("slave turn to send mode \n");
    HAL_SPI_Init(&SPI0_OBJ);
    /* Transmit data */
    HAL_SPI_SendData(&SPI0_OBJ, slave_send_buf, slave_receive_send_cnt,0);
    ...
    HAL_SPI_SendReceiveData(&SPI0_OBJ, slave_send_buf, slave_receive_send_cnt,
                        slave_receive_buf, slave_receive_send_cnt);
    HAL_SPI_DeInit(&SPI0_OBJ);
    APP_LOG_INFO("slave receive data at quasi mode: \n");
    for (uint16_t cnt = 0; cnt < slave_receive_send_cnt; cnt++) {
        APP_LOG("0x%04x,", slave_receive_buf[cnt]);
        if (cnt % 0x10 == 0xf) {
            APP_LOG("\n");
