...
static void spi_tx_callback(SPI_CbFlagOpt flag,uint16_t* pBuf,uint16_t size)
{
    ...
    APP_LOG_INFO("slave send done! sent %d data as interrupt mode\n", size);
}

static void spi_rx_callback(SPI_CbFlagOpt flag,uint16_t* pBuf,uint16_t size)
{
    slave_receive_done_flag = true;
    APP_LOG_INFO("slave receive done! receive %d data as interrupt mode\n", size);
}
...
static void spi_slave_receive_send_interrupt_test(void)
{
    ...
    HAL_SPI_ReceiveData_INT(&SPI0_OBJ, slave_receive_buf, slave_receive_send_cnt, spi_rx_callback);
    ...
    HAL_SPI_Init(&SPI0_OBJ);
    /* Transmit data */
    HAL_SPI_SendData_INT(&SPI0_OBJ, slave_send_buf, slave_receive_send_cnt, spi_tx_callback);
    ...
    for (uint16_t cnt = 0; cnt < slave_receive_send_cnt; cnt++) {
        ...
        slave_receive_buf[cnt] = 0;
    }
    HAL_SPI_SendReceiveData_INT(&SPI0_OBJ, slave_send_buf, slave_receive_send_cnt, spi_tx_callback,
...
{
    spi_slave_receive_send_interrupt_test();
