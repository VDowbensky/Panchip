...
static volatile bool master_send_done_flag = false;
static volatile bool master_receive_done_flag = false;
...
static void spi_master_send_receive_dma_test(void)
{
    uint16_t master_receive_buf[160] = {0, };
    uint16_t master_send_receive_cnt = sizeof(master_send_buf) / sizeof(master_send_buf[0]);

    /* Transmit data */
    HAL_SPI_SendData_DMA(&SPI0_OBJ, master_send_buf, master_send_receive_cnt, spi_tx_callback);
    while(false == master_send_done_flag){}
    master_send_done_flag = false;
    HAL_SPI_DeInit(&SPI0_OBJ);
    APP_LOG_INFO("master send data: \n");
    for (uint16_t cnt = 0; cnt < master_send_receive_cnt; cnt++) {
        APP_LOG("0x%04x,", master_send_buf[cnt]);
        if (cnt % 0x10 == 0xf) {
            APP_LOG("\n");
        }
    }

    APP_LOG_INFO("master turn to receive mode \n");
    HAL_SPI_Init(&SPI0_OBJ);
    for (uint16_t cnt = 0; cnt < master_send_receive_cnt; cnt++) {
        master_send_buf[cnt] = 0;
        master_receive_buf[cnt] = 0xaa;
    }

    HAL_SPI_SendReceiveData_DMA(&SPI0_OBJ, master_send_buf, master_send_receive_cnt, spi_tx_callback,
                                master_receive_buf, master_send_receive_cnt, spi_rx_callback);
    while(false == master_receive_done_flag){}
    master_receive_done_flag = false;
    HAL_SPI_DeInit(&SPI0_OBJ);
    ...
    APP_LOG_INFO("master turn to quasi mode \n");
...
{
    spi_master_send_receive_dma_test();
