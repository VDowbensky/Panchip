...
static const char *device_name = "b+EMIE Elfy";
...
void notify_data(uint8_t *data, uint16_t len)
{
	...
	if (notify_state) {
		...
		rc = ble_gatts_notify_custom(conn_handle, rgb_c2003_handle, om);
		assert(rc == 0);
	}
}
...
static int app_ble_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_SUBSCRIBE:
        ...
        if (event->subscribe.attr_handle == rgb_c2003_handle) {
            notify_state = event->subscribe.cur_notify;
        } else if (event->subscribe.attr_handle != rgb_c2003_handle) {
            notify_state = event->subscribe.cur_notify;
        }
        break;
