...
static int app_ble_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_SUBSCRIBE:
        ...
        if (event->subscribe.attr_handle == conn_handle) {
            notify_state = event->subscribe.cur_notify;
        }
		else if (event->subscribe.attr_handle != conn_handle) {
            notify_state = event->subscribe.cur_notify;
        }
        break;
...
void ble_svc_spp_uart_write(uint8_t *data, uint16_t len)
{
...
void ble_uart_notify(uint8_t *data, uint16_t len)
{
	...
	if (notify_state) {
		rc = ble_svc_spp_send_notify(conn_handle, data, len);
		assert(rc == 0);
	}
}
