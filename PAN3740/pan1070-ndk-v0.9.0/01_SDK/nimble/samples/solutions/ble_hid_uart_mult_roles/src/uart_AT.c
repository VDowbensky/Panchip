...
void UART_HandleProc(UART_T *UARTx)
{
	...
	case UART_EVENT_TIMEOUT:
		...
		if(RX_Sendata[0] == 0x5A)
		{
			...
			//APP_LOG_DATA(send_data, USART_RX_CNT);
...
void uart_at_handler(void)
{
	...
	case AT_SEND_DATA:
		...
		if(send_data[1] & (0x1 << 0)) {
			APP_LOG_INFO("AT_WRITE_PERI\n");
			app_ble_central_write(&send_data[2], send_data_len - 2);
		}
