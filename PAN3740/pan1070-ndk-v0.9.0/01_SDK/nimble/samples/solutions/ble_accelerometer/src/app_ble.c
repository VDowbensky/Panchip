...
/**@brief sensor data notify timer. */
...
// acc sensor data tx API
static void app_ble_sensor_tx_timer_start(void);
static void app_ble_sensor_tx_timer_stop(void);
...
static int app_ble_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_SUBSCRIBE:
        ...
        if (event->subscribe.attr_handle == c2003_handle) {
            notify_state = event->subscribe.cur_notify;
			if(notify_state) {
				app_ble_sensor_tx_timer_start();
			} else {
				app_ble_sensor_tx_timer_stop();
			}
        }
		else if (event->subscribe.attr_handle != c2003_handle) {
            notify_state = event->subscribe.cur_notify;
            app_ble_sensor_tx_timer_stop();
        }
        break;
...
static void app_ble_sensor_tx_timer_start(void)
{
    ...
    rc = ble_npl_callout_reset(&sensor_notify_timer, pdMS_TO_TICKS(current_period_time));
    APP_ASSERT(rc == 0);
}
...
{
	ble_npl_callout_init(&sensor_notify_timer, (struct ble_npl_eventq *)nimble_port_get_dflt_eventq(),
						  period_notify_cb, NULL);
}
