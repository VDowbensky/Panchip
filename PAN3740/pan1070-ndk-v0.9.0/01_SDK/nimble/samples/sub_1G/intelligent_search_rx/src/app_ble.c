...
static const char *device_name = "b+sf rx";

/**@brief tx period timer. */
...
void notify_data(uint8_t *data, uint16_t len)
{
	...
	if (notify_state) {
		...
		rc = ble_gatts_notify_custom(conn_handle, sub_1g_handle, om);
		assert(rc == 0);
	}
}
...
static int app_ble_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_SUBSCRIBE:
		...
		if ((event->subscribe.attr_handle == sub_1g_handle) && (!notify_state)) {
			rf_set_mode(RF_MODE_STB3);
			rf_refresh();
			app_ble_hr_tx_timer_stop();
		} else if ((event->subscribe.attr_handle == sub_1g_handle) && (notify_state)) {
			trx_init();
		}
        break;
...
#else
	struct ble_gap_upd_params upd_params = {
		.itvl_min = BLE_GAP_CONN_ITVL_MS(1000),
		.itvl_max = BLE_GAP_CONN_ITVL_MS(1000),
		.latency  = 0,
		.supervision_timeout = 500,
	};
#endif
...
void app_ble_hr_tx_timer_start(void)
{
    ...
    rc = ble_npl_callout_reset(&trx_period_tx_timer, pdMS_TO_TICKS(current_period_time));
    APP_ASSERT(rc == 0);
}
...
static void period_notify_init(void)
{
	ble_npl_callout_init(&trx_period_tx_timer, (struct ble_npl_eventq *)nimble_port_get_dflt_eventq(),
						  trx_period_tx_cb, NULL);
}
...
static void app_ble_advertise_start(void)
{
    ...
    adv_params.itvl_max = BLE_GAP_ADV_ITVL_MS(30 + 100);
