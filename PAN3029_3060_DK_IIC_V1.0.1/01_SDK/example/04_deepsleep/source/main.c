 ...
 **********************************************************************************
 * @file    main.c
 * @brief   本示例演示了如何使用PAN3029/3060交替进行数据发送和深度睡眠的功能。
 * @version V1.0.1
 * @date    2025-08-18
 * @author  Panchip Team
 ...
 *          - 另一块开发板烧录本示例程序，进行数据发送和深度睡眠的验证。
 * @note    PAN3029/3060在进入深度睡眠(deepsleep）后，RF模块的功耗会降低0.5uA以下，
 *          芯片内所有的寄存器和状态会丢失，退出睡眠后，RF模块会自动恢复到之前的状态。
...
int32_t main(void)
{
    ...
    printf("Tx with deepsleep test start.\r\n");
    ...
    while(1)
    {
        ...
        printf("Enter deepsleep status.\r\n");
        RF_EnterDeepsleepState(); /* 进入深度睡眠状态 */
        SysTick_Delay(500);       /* 延时500ms */
        printf("Exit deepsleep status.\r\n");
        RF_Init();                /* 重新初始化RF */
