...
/* I2C ACK 最大超时计数宏定义，具体需要根据你的硬件来设置 */
#define ACK_TIMEOUT_DEF 1000

/* TODO: 根据你的硬件实现 
 *  - I2C_SCK引脚配置为推挽输出
 *  - I2C_SDA引脚配置为推挽输出或输入带上拉电阻模式
 *  注意：使用I2C时，CS引脚必须外部拉高或者浮空
 */
#define I2C_SCK_LOW     PORT_ResetBits(SOFT_I2C_SCL_PORT, SOFT_I2C_SCL_PIN)                                         /* 将I2C_SCK引脚拉低 */
#define I2C_SCK_HIGH    PORT_SetBits(SOFT_I2C_SCL_PORT, SOFT_I2C_SCL_PIN)                                           /* 将I2C_SCK引脚拉高 */
#define I2C_SDA_LOW     PORT_ResetBits(SOFT_I2C_SDA_PORT, SOFT_I2C_SDA_PIN)                                         /* 将I2C_SDA引脚拉低 */
#define I2C_SDA_HIGH    PORT_SetBits(SOFT_I2C_SDA_PORT, SOFT_I2C_SDA_PIN)                                           /* 将I2C_SDA引脚拉高 */
#define I2C_SDA_STATUS  PORT_GetBit(SOFT_I2C_SDA_PORT, SOFT_I2C_SDA_PIN)                                            /* 获取I2C_SDA电平状态 */
#define I2C_SDA_OUTPUT  gpioe_set_out(gpioe_get_ctrl_base(SOFT_I2C_SDA_PORT), get_bit_index(SOFT_I2C_SDA_PIN))      /* 设置I2C_SDA引脚为输出模式 */
#define I2C_SDA_INPUT   gpioe_set_in(gpioe_get_ctrl_base(SOFT_I2C_SDA_PORT), get_bit_index(SOFT_I2C_SDA_PIN))       /* 设置I2C_SDA引脚为输入模式 */

/* 用于软I2C的延时实现(根据硬件平台调整)，且防止编译器优化 */
...
} while(0) /* 使用volatile变量防止编译器优化，更改i2c_delay_counter的值可调整I2C速率 */
 ...
 */
#define RF_RESET_PIN_LOW()  PORT_ResetBits(GPIO_PORT_RF_RST, GPIO_PIN_RF_RST) /* 设置复位引脚为低电平 */
#define RF_RESET_PIN_High() PORT_SetBits(GPIO_PORT_RF_RST, GPIO_PIN_RF_RST)   /* 设置复位引脚为高电平 */
...
#define USE_RF_RST_GPIO            0 /* 是否使用MCU引脚控制RF复位功能(0:不使用, 1: 使用) */
