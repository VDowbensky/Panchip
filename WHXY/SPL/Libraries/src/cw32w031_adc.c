...
void ADC_SetSingleCh(uint32_t enAdcSampCh)
{
    ...
    if ((enAdcSampCh == ADC_VddaDiV3Input) || (enAdcSampCh == ADC_TsInput) || (enAdcSampCh == ADC_Vref1P2Input))
    {
...
void ADC_SetSerialCh(uint32_t enAdcSampCh)
{
    ...
    if ((enAdcSampCh == ADC_SqrVddaDiV3) || (enAdcSampCh == ADC_SqrTs) || (enAdcSampCh == ADC_SqrVref1P2))
    {
        REGBITS_SET(CW_ADC->CR0, ADC_CR0_BUF_Msk);
    }
