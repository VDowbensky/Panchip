...
#ifndef __CW32W031_WWDT_H
#define __CW32W031_WWDT_H
...
#define WWDT_GET_POV_STATUS(VALUE)       \
  {                                      \
    (VALUE) = (FlagStatus)(CW_WWDT->SR); \
  }
#define WWDT_CLR_POV_STATUS()                  \
  {                                            \
    REGBITS_CLR(CW_WWDT->SR, WWDT_SR_POV_Msk); \
  }
#define WWDT_ENABLE_IT()                        \
  {                                             \
    REGBITS_SET(CW_WWDT->CR1, WWDT_CR1_IE_Msk); \
  }
#define WWDT_FEED(VALUE)                                      \
  {                                                           \
    REGBITS_MODIFY(CW_WWDT->CR0, WWDT_CR0_WCNT_Msk, (VALUE)); \
  }
