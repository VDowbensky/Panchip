...
static uint8_t cad_state = RADIO_FLAG_IDLE;
...
/**
 * @brief RF PAN3028_irq_handler OnRadioRxPlhdDone callbact,it will use in Plhd Mode
 ...
 void rf_rx_plhddone_event( uint8_t *payload, uint16_t size )
{
	...
	/**
 * @brief RF PAN3028_irq_handler OnRadioRxDone callbact,it will use in PAN3028_RX_SINGLE/PAN3028_RX_SINGLE_TIMEOUT/PAN3028_RX_CONTINOUS Mode
 * @param[in] <payload> recv packet
 * @param[in] <size> the length of recv packet
...
/**
 * @brief RF PAN3028_irq_handler OnRadioTxDone callbact,it will use in tx Mode
 * @param[in] <none> 
 * @return none
 */
 ...
 * @return none
 */
 void rf_cad_done_event(void)
{
	rf_set_cad_flag(RADIO_FLAG_CAD);
}

uint8_t rf_get_cad_flag(void)
{
	return cad_state;
}

void rf_set_cad_flag(uint8_t status)
{
    cad_state = status;
}
...
uint32_t check_cad_tx_inactive(void)
{
	...
	uint32_t getOnepreamble(uint32_t bw,uint32_t sf)
{
	switch(bw)
	{
		case 6:
