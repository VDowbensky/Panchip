...
/* Includes ------------------------------------------------------------------*/
#include "../inc/interrupts_cw32w031.h"
 ...
 */
static uint8_t data[10] = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A};
...
void UART1_IRQHandler(void)
{
    ...
    {
        u8RxData = USART_ReceiveData_8bit(CW_UART1);
        USART_ClearITPendingBit(CW_UART1, USART_IT_RC);
    }
    if(IsFifoFull(&uart_rx_fifo) == FALSE)
    {
        FifoPush(&uart_rx_fifo, u8RxData);
    }
    else
    {
		}
