...
uint8_t TestModeBuffer[TEST_MODE_BUFFER_LEN] = {0};
...
void key_set_test_mode1(void)
{
	...
	if(key_set_test_mode1_flag == 2){
		uint8_t tmpreg;
		tmpreg = rf_read_spec_page_reg(PAGE3_SEL, 0x12);
		rf_write_spec_page_reg(PAGE3_SEL, 0x12, (0x16|(tmpreg&0x08)));
	}else{
		set_test_mode1_reg();
	}
...
void process_rf_events(void)
{
    ...
    if((flag == RADIO_FLAG_RXERR)&&(key_set_test_mode1_flag == 1))
    {
        rf_set_recv_flag(RADIO_FLAG_IDLE);

		if(memcmp(RxDoneParams.TestModePayload, TestModeBuffer, TEST_MODE_BUFFER_LEN) == 0)
		{
			DDL_Printf("RxTestMode1 Recv\r\n");
			BSP_LED_Toggle();
			rf_rx_done_routine();
		}else{
			
			DDL_Printf("RxTestMode1Err,Payload:\r\n");
			for(i = 0; i < TEST_MODE_BUFFER_LEN; i++)
			{
				DDL_Printf("0x%02x ", RxDoneParams.TestModePayload[i]);
			}
			DDL_Printf("\r\n");			
		}
    }
...
extern uint8_t vbat_low_flag;
extern uint8_t vbat_charging_flag;
void vbat_scan(void)
{
    if(PORT_GetBit(PortC, Pin14) == 0)
    {
        rf_delay_us(150);
        if(PORT_GetBit(PortC, Pin14) == 0)
        {
            vbat_charging_flag = 1;
            PORT_ResetBits(PortH, Pin02);
        }
    } else {
        vbat_charging_flag = 0;
        if(!vbat_low_flag)
        {
            PORT_SetBits(PortH, Pin02);
        }
